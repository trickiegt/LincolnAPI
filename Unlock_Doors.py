#Send Command to Unlock Vehicle
import requests
import json
import time
...
print("Unlock Command ID is:", results['COMMANDID'])
...
while complete == False:
    ...
    if results['Entries']['Entry']['status'] != "QUEUED":
        if results['Entries']['Entry']['status'] == "COMPLETED":
            print("Unlock Command Executed Successfully")    
            complete = True
        else:
            print("Unlock Command Failed -", results['Entries']['Entry']['status'])    
            complete = True